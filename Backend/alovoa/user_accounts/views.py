...
from .models import Profile,Preference,FamilyName, UserLike,SuccessStory,ContactUs,ContactDetails, CreateSubscription,UploadedImages,Religion, ProfilePicture,Subscription,CustomUser, Community,State,District
from .serializers import (CustomUserSerializer,SuccessStorySerializer, ProfileSerializer,
                          PreferenceSerializer, FamilyNameSerializer,UserLikeSerializer,
                          UploadedImagesSerializer, SubscriptionSerializer,
                          CommunitySerializer,ReligionSerializer,StateSerializer,
                          DistrictSerializer, ProfilePictureSerializer,ContactUsSerializer, ContactDetailsSerializer,CreateSubscriptionSerializer)
import cv2
    ...
    def get(self, request):
        ...
        if profile_query_params_exist:
            ...
            if caste is not None:
                profile_queryset = profile_queryset.filter(caste=caste)
...
class CustomUserSearchByCustomIDView(generics.ListAPIView):
    serializer_class = CustomUserSerializer

    def get_queryset(self):
        custom_id = self.request.query_params.get('custom_id')
